...
"""
Monitor Recursivo de Visuales UCLV
Rastrea actualizaciones en la red universitaria e informa cambios vía Telegram.
"""
...
def enviar_alerta_telegram(mensaje: str):
    if not TELEGRAM_TOKEN or not CHAT_ID:
        print("Telegram no configurado para el Monitor.")
        return
...
def mapear_servidor_recursivo(url_actual: str, profundidad_actual: int = 0) -> List[str]:
    """Rastrea de forma profunda hasta el límite establecido recolectando URLs estables"""
    ...
    try:
        ...
        if estado_previo.get('hash') != hash_actual:
            ...
            mensaje = (
                ...
                f"➕ *Nuevas entradas aproximadas:* {nuevas_entradas}\n"
                f"📁 *Estructura:* Escaneo completado a {MAX_DEPTH} niveles."
            )
            ...
            with open(ESTADO_FILE, 'w') as f:
                json.dump({
                    ...
                    'items_count': len(urls_encontradas)
                }, f, indent=2)
            print(f"Cambios guardados. Total URLs del mapa: {len(urls_encontradas)}")
        else:
            print("El servidor no presenta modificaciones desde el último ciclo.")
            
    except Exception as e:
        print(f"Error crítico en el demonio del monitor: {e}")
